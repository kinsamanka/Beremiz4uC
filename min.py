...
class MINTransport:
    """
    Handle MIN Transport. Runs as a polled system; typically will be subclassed
    to run in a threaded environment that puts thread locks around API calls.
    """
    ...
    def __init__(
        ...
        :param idle_timeout_ms: Time before connection assumed to have been
                            lost and retransmissions stopped
    ...
    def _send_ack(self):
        ...
        ack_frame = MINFrame(min_id=self.ACK, seq=self._rn, payload=bytes(
            [self._rn]), transport=True, ack_or_reset=True)
    ...
    def transport_reset(self):
        """
        Sends a RESET to the other side to say that we are going away and
        clears out the FIFO and receive queues
    ...
    def send_frame(self, min_id: int, payload: bytes):
        """
        Sends a MIN frame with a given ID directly on the wire. Will be
        silently discarded if any line noise.
        ...
        on_wire_bytes = self._on_wire_bytes(frame=frame)
        min_logger.info(
            "Sending MIN frame, min_id={}, payload={}".format(
                min_id, bytes_to_hexstr(payload)))
        min_logger.debug(
            "Sending MIN frame, on wire bytes={}".format(
                bytes_to_hexstr(on_wire_bytes)))
        self._serial_write(on_wire_bytes)

    def queue_frame(self, min_id: int, payload: bytes):
        """
        Queues a MIN frame for transmission through the transport protocol.
        Will be retransmitted until it is delivered or the connection has
        timed out.
    ...
    def _min_frame_received(
            ...
            min_seq: int):
        """
        Handle a received MIN frame. Because this runs on a host with plenty
        of CPU time and memory we stash out-of-order frames and send negative
        acknowledgements (NACKs) to ask for missing ones. This greatly improves
        the performance in the presence of line noise: a dropped frame will be
        specifically requested to be resent and then the stashed frames
        appended in the right order.

        Note that the automatic retransmit of frames must be tuned carefully
        so that a window + a NACK received + retransmission of missing frames
        + ACK for the complete set is faster than the retransmission timeout
        otherwise there is unnecessary retransmission of frames which
        wastes bandwidth.

        The embedded version of this code does not implement NACKs: generally
        the MCU will not have enough memory to stash out-of-order
        frames for later reassembly.
        """
        min_logger.debug(
            "MIN frame received @{}: min_id_control=0x{:02x}, min_seq={}".format(
                time(), min_id_control, min_seq))
        self._last_received_anything_ms = self._now_ms()
        if min_id_control & 0x80:
            if min_id_control == self.ACK:
                min_logger.debug("Received ACK")
                # The ACK number indicates the serial number of the next packet
                # wanted, so any previous packets can be marked off
                ...
                if number_acked <= number_in_window:
                    ...
                    if number_in_window > 0:
                        min_logger.warning(
                            f"Spurious ACK: self._sn_min={self._sn_min}, "
                            f"self._sn_max={self._sn_max}, min_seq={min_seq}, "
                            f"payload[0]={min_payload[0]}")
                ...
                if min_seq == self._rn:
                    min_logger.debug(
                        f"MIN application frame received @{time()} "
                        f"(min_id={min_id_control & 0x3f} seq={min_seq})")
                    ...
                    while self._rn in self._stashed_rx_dict:
                        ...
                        min_logger.debug(
                            f"MIN application stashed frame recovered @{time()}"
                            f" (self._rn={self._rn} "
                            f"min_id={stashed_frame.min_id} "
                            f"seq={stashed_frame.seq})")
                        del self._stashed_rx_dict[self._rn]
                        self._rx_list.append(stashed_frame)
                        self._rn = (self._rn + 1) & 0xff
                        if self._rn == self._nack_outstanding:
                            # The missing frames we asked for have joined up
                            # with the main sequence
                            self._nack_outstanding = None

                    # If there are stashed frames left then it means that the
                    # stashed ones have missing frames in the sequence
                    if self._nack_outstanding is None and len(
                            self._stashed_rx_dict) > 0:
                        ...
                        if (earliest_seq -
                                self._rn) & 0xff < self.rx_window_size:
                        ...
                        self._send_ack()
                    min_logger.debug(
                        "Sending ACK for min ID={} with self._rn={}".format(
                            min_id_control & 0x3f, self._rn))
                else:
                    # If the frames come within the window size in the future
                    # sequence range then we accept them and assume some were
                    # missing
                    # (They may also be duplicates, in which case we store them
                    # over the top of the old ones)
                    if (min_seq - self._rn) & 0xff < self.rx_window_size:
                        # We want to only NACK a range of frames once, not each
                        # time otherwise we will overload with retransmissions
                        if self._nack_outstanding is None:
                            # If we are missing specific frames then send a
                            # NACK to specifically request them
                            min_logger.debug(
                                f"Sending NACK for min ID="
                                f"{min_id_control & 0x3f} with seq={self._rn} "
                                f"to={min_seq}")
                        ...
                        min_logger.debug(
                            f"MIN application frame stashed @{time()} "
                            f"(min_id={min_id_control & 0x3f}, seq={min_seq})")
                    else:
                        min_logger.warning(
                            f"Frame stale? Discarding @{time()} "
                            f"(min_id={min_id_control & 0x3f}, seq={min_seq})")
                        if min_seq in self._stashed_rx_dict and \
                          min_payload != self._stashed_rx_dict[min_seq].payload:
                            min_logger.error(
                                "Inconsistency between frame contents")
                        # Out of range (may be an old retransmit duplicate that
                        # we don't want) - throw it away
        ...
        for byte in data:
            if self._rx_header_bytes_seen == 2:
                ...
                if byte == self.STUFF_BYTE:
                    ...
                    continue
                # By here something must have gone wrong, give up on this frame
                # and look for new header
                self._rx_frame_state = self.SEARCHING_FOR_SOF
                continue
            ...
            if self._rx_frame_state == self.SEARCHING_FOR_SOF:
                ...
                if self._rx_frame_id_control & 0x80:
                    computed_checksum = self._crc32(bytearray(
                        [self._rx_frame_id_control, self._rx_frame_seq,
                         self._rx_control]) + self._rx_frame_buf)
                else:
                    computed_checksum = self._crc32(bytearray(
                        [self._rx_frame_id_control, self._rx_control])
                        + self._rx_frame_buf)

                if self._rx_frame_checksum != computed_checksum:
                    min_logger.warning(
                        "CRC mismatch (0x{:08x} vs 0x{:08x}), frame dropped".format(
                            self._rx_frame_checksum, computed_checksum))
                ...
                if byte == self.EOF_BYTE:
                    ...
                    self._min_frame_received(
                        min_id_control=self._rx_frame_id_control, min_payload=bytes(
                            self._rx_frame_buf), min_seq=self._rx_frame_seq)
                else:
    ...
    def _on_wire_bytes(self, frame: MINFrame) -> bytes:
        """
        Get the on-wire byte sequence for the frame, including stuff bytes
        after every 0xaa 0xaa pair
        """
        if frame.is_transport:
            prolog = bytes([frame.min_id | 0x80, frame.seq,
        ...
        stuffed = bytearray(
            [self.HEADER_BYTE, self.HEADER_BYTE, self.HEADER_BYTE])
    ...
    def _crc32(checksummed_data: bytearray, start=0xffffffff):
        """
        The 'manual' implementation is left here as a guide to implementing
        this on microcontrollers. It's cross-checked with the standard
    ...
    def poll(self):
        """
        Polls the serial line, runs through MIN, sends ACKs, handles
        retransmits where ACK has gone missing.

        :return: array of accepted MIN frames
        """
        remote_connected = (
            self._now_ms() -
            self._last_received_anything_ms) < self.idle_timeout_ms
        remote_active = (
            self._now_ms() -
            self._last_received_frame_ms) < self.idle_timeout_ms
        ...
        window_size = (self._sn_max - self._sn_min) & 0xff
        if window_size < self.max_window_size and len(
                self._transport_fifo) > window_size:
            ...
            frame.last_sent_time = self._now_ms()
            min_logger.debug(
                "Sending new frame id={} seq={} len={} payload={}".format(
                    frame.min_id, frame.seq, len(
                        frame.payload), bytes_to_hexstr(
                        frame.payload)))
            ...
            if window_size > 0 and remote_connected:
                ...
                if self._now_ms() - oldest_frame.last_sent_time > self.frame_retransmit_timeout_ms:
                    min_logger.debug(
                        "Resending old frame id={} seq={}".format(
                            oldest_frame.min_id, oldest_frame.seq))
...
class MINTransportSerial(MINTransport):
    """
    Bound to Pyserial driver. But not thread safe: must not call poll()
    ...
    def __init__(self, port, baud=115200, loglevel=ERROR):
        ...
        try:
            ...
            raise MINConnectionError(
                "Transport MIN cannot open port '{}'".format(port))
        super().__init__(loglevel=loglevel)


class ThreadsafeTransportMINSerialHandler(MINTransportSerial):
    """
    This class wraps the API calls with thread locks to prevent concurrent
    ...
    def __init__(self, port, baud=115200, loglevel=ERROR):
        super().__init__(port=port, baud=115200, loglevel=loglevel)
