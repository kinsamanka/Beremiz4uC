...
// stuff bytes 3 header bytes, ID/control byte, length byte, seq byte,
// 4 byte CRC, EOF byte
...
static void on_wire_bytes(struct min_context *self, uint8_t id_control,
                          uint8_t seq, uint8_t const *payload_base,
                          uint16_t payload_offset, uint16_t payload_mask,
                          uint8_t payload_len)
{
    uint8_t n, i;
    uint32_t checksum;
...
static void transport_fifo_pop(struct min_context *self)
{
    ...
    struct transport_frame *frame =
        &self->transport_fifo.frames[self->transport_fifo.head_idx];
...
static struct transport_frame *transport_fifo_push(struct min_context *self,
                                                   uint16_t data_size)
{
    ...
    if (self->transport_fifo.n_frames < TRANSPORT_FIFO_MAX_FRAMES) {
        ...
        if (self->transport_fifo.n_ring_buffer_bytes <=
            TRANSPORT_FIFO_MAX_FRAME_DATA - data_size) {
            self->transport_fifo.n_frames++;
            if (self->transport_fifo.n_frames >
                self->transport_fifo.n_frames_max) {
            ...
            if (self->transport_fifo.n_ring_buffer_bytes >
                self->transport_fifo.n_ring_buffer_bytes_max) {
            ...
            self->transport_fifo.ring_buffer_tail_offset &=
                TRANSPORT_FIFO_SIZE_FRAME_DATA_MASK;
...
static struct transport_frame *transport_fifo_get(struct min_context *self,
                                                  uint8_t n)
{
    uint8_t idx = self->transport_fifo.head_idx;
    return &self->transport_fifo.
        frames[(idx + n) & TRANSPORT_FIFO_SIZE_FRAMES_MASK];
}

// Sends the given frame to the serial line
static void transport_fifo_send(struct min_context *self,
                                struct transport_frame *frame)
{
    min_debug_print("transport_fifo_send: min_id=%d, seq=%d, payload_len=%d\n",
                    frame->min_id, frame->seq, frame->payload_len);
    on_wire_bytes(self, frame->min_id | (uint8_t) 0x80U, frame->seq,
                  payloads_ring_buffer, frame->payload_offset,
                  TRANSPORT_FIFO_SIZE_FRAME_DATA_MASK, frame->payload_len);
...
static void send_ack(struct min_context *self)
{
    ...
    if (ON_WIRE_SIZE(0) <= min_tx_space(self->port)) {
        on_wire_bytes(self, ACK, self->transport_fifo.rn,
                      &self->transport_fifo.rn, 0, 0xffU, 1U);
...
bool min_queue_frame(struct min_context *self, uint8_t min_id,
...
bool min_queue_has_space_for_frame(struct min_context *self,
                                   uint8_t payload_len)
{
    return self->transport_fifo.n_frames < TRANSPORT_FIFO_MAX_FRAMES &&
        self->transport_fifo.n_ring_buffer_bytes <=
        TRANSPORT_FIFO_MAX_FRAME_DATA - payload_len;
}
...
static struct transport_frame *find_retransmit_frame(struct min_context *self)
{
    ...
    uint8_t i;
    uint8_t window_size =
        self->transport_fifo.sn_max - self->transport_fifo.sn_min;
    ...
    struct transport_frame *oldest_frame =
        &self->transport_fifo.frames[self->transport_fifo.head_idx];
    ...
    for (i = 0; i < window_size; i++) {
        uint32_t elapsed =
            now - self->transport_fifo.frames[idx].last_sent_time_ms;
...
// This runs the receiving half of the transport protocol, acknowledging
// frames received, discarding duplicates received, and handling RESET requests.
static void valid_frame_received(struct min_context *self)
{
    ...
    switch (id_control) {
    case ACK:
        // If we get an ACK then we remove all the acknowledged frames with
        // seq < rn. The payload byte specifies the number of NACKed frames:
        // how many we want retransmitted because they have gone missing.
        // But we need to make sure we don't accidentally ACK too many
        // because of a stale ACK from an old session
        ...
        if (num_acked <= num_in_window) {
            ...
            for (i = 0; i < num_nacked; i++) {
                struct transport_frame *retransmit_frame =
                    &self->transport_fifo.frames[idx];
        ...
        break;
    case RESET:
        // If we get a RESET demand then we reset the transport protocol
        // (empty the FIFO, reset the sequence numbers, etc.)
        // We don't send anything, we just do it. The other end can send
        // frames to see if this end is alive (pings, etc.) or just wait 
        // to get application frames.
    ...
    default:
        if (id_control & 0x80U) {
            ...
            if (seq == self->transport_fifo.rn) {
                ...
                // ACKs are short (should be about 9 microseconds to send on
                // the wire) and this will cut the latency down.
                // We also periodically send an ACK in case the ACK was lost,
                // and in any case frames are re-sent.
                send_ack(self);
                ...
                min_debug_print
                    ("Incoming app transport frame seq=%d, id=%d, payload len=%d\n",
                     seq, id_control & (uint8_t) 0x3fU, payload_len);
                min_application_handler(id_control & (uint8_t) 0x3fU, payload,
                                        payload_len, self->port);
            } else {
                // Discard this frame because we aren't looking for it: it's
                // either a dupe because it was retransmitted when our ACK 
                // didn't get through in time, or else it's further on in the
                // sequence and others got dropped.
                self->transport_fifo.sequence_mismatch_drop++;
                min_debug_print
                    ("Received mismatched frame seq=%d, looking for seq=%d\n",
...
static void rx_byte(struct min_context *self, uint8_t byte)
{
    ...
    case RECEIVING_LENGTH:
        ...
        if (self->rx_frame_length > 0) {
            ...
            if (self->rx_frame_length <= MAX_PAYLOAD) {
                ...
                min_debug_print
                    ("Dropping frame because length %d > MAX_PAYLOAD %d",
    ...
    default:
        ...
        min_debug_print("Received byte 0x%02X in invalid state %d", byte,
                        self->rx_frame_state);
        self->rx_frame_state = SEARCHING_FOR_SOF;
        break;
    }
...
void min_poll(struct min_context *self, uint8_t const *buf, uint32_t buf_len)
{
    ...
    bool remote_connected =
        (now - self->transport_fifo.last_received_anything_ms <
         TRANSPORT_IDLE_TIMEOUT_MS);
    bool remote_active =
        (now - self->transport_fifo.last_received_frame_ms <
         TRANSPORT_IDLE_TIMEOUT_MS);
    ...
    if ((window_size < TRANSPORT_MAX_WINDOW_SIZE)
        && (self->transport_fifo.n_frames > window_size)) {
        ...
        if ((window_size > 0) && remote_connected) {
            ...
            if (now - oldest_frame->last_sent_time_ms >=
                ...
                if (ON_WIRE_SIZE(oldest_frame->payload_len) <=
                    min_tx_space(self->port)) {
    ...
    if (now - self->transport_fifo.last_sent_ack_time_ms >
...
void min_init_context_validate(struct min_context *self, uint8_t port,
                               void *p_rx_frame_checksum)
#else
...
#endif
{
    ...
    assert(self != 0);
#ifdef VALIDATE_MAX_PAYLOAD
    // check the provided buffer is large enough. This could be false if
    // MIN_PAYLOAD is defined differently when compiling calling code
    // and this code.
    assert((void *)(self->rx_frame_payload_buf + MAX_PAYLOAD) <=
           p_rx_frame_checksum);
#endif
#endif
    // Initialize context
...
void min_send_frame(struct min_context *self, uint8_t min_id,
                    uint8_t const *payload, uint8_t payload_len)
{
    if ((ON_WIRE_SIZE(payload_len) <= min_tx_space(self->port))) {
        on_wire_bytes(self, min_id & (uint8_t) 0x3fU, 0, payload, 0, 0xffffU,
                      payload_len);
    }
}
...
{
    /* CALLBACK: Start of frame transmission. */
}

__attribute__((weak))
void min_tx_finished(uint8_t port)
{
    /* CALLBACK: End of frame transmission. */
}
